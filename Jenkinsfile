pipeline {
	...
	stages {
		stage('Setup Docker Agent') {
			...
			environment {
				DOCKER_HUB_REPO = "nmm131/terraform-kubernetes-jenkins-flask"
			}
			stages {
			...
			steps {
				node('master') {
					script {
						...
						ansiblePlaybook credentialsId: 'ansible', disableHostKeyChecking: true, installation: 'Ansible', inventory: '/etc/ansible/hosts', playbook: '/home/master/terraform-kubernetes-jenkins-flask/playbook-deploy-app.yaml'
					}
				}
			}
		}
